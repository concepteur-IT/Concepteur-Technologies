...
>
  <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 relative">
    ...
    <div className="relative z-10 flex flex-col justify-center min-h-[400px] lg:min-h-[500px] ">
      ...
      <div className={`${HEADING_BLOCK_SPACING} max-w-xl bg-white  backdrop-blur-sm p-6 md:p-10 custom-notch-tl-br shadow-lg `}>
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
          ...
          <div className="flex flex-wrap gap-3 mb-10 md:mb-16 border-b border-gray-100 pb-8">
            {filters.map((filter) => (
              <button
                key={filter}
                onClick={() => setActiveFilter(filter)}
                className={`px-6 py-3 text-xs font-medium uppercase tracking-wider transition-all duration-300 border ${
                  activeFilter === filter
          ...
          {filteredStudies.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {filteredStudies.map((item, index) => {
                ...
                const placeholderImages = ["/home/cap-web.png", "/home/cap-mobile.png", "/home/cap-backend.png"];
                const imgPlaceholder = placeholderImages[index % placeholderImages.length];
...
<motion.article
                    ...
                    className={`group flex flex-col transition-all duration-500 overflow-hidden bg-white rounded-3xl ${
                      isActive
                  ...
                  >
                    ...
                    >
                      {/* New Image Block Inside Card */}
                      <div className="relative w-full aspect-[4/3] bg-gray-100 overflow-hidden">
                        <Image 
                          src={imgPlaceholder} 
                          fill 
                          alt={item.title} 
                          className="object-cover transition-transform duration-[1200ms] ease-[0.22,1,0.36,1] group-hover:scale-110"
                        />
                        <div className="absolute inset-0 bg-black/0 group-hover:bg-black/5 transition-colors duration-500" />
                      </div>

                      <div className="p-6 md:p-8 flex flex-col flex-1">
                        <span className="text-[11px] uppercase tracking-[0.2em] text-gray-500 font-medium mb-4">
                          {item.tag || "Case Study"}
                        </span>
                        <h3 className="text-2xl font-light tracking-tight text-black leading-snug mb-2">
                          {item.title}
                        </h3>
                    ...
                    <div className="mt-auto bg-gray-50 group-hover:bg-black transition-colors duration-500">
                      <Link
                        href={`/case-studies/${item.slug}`}
                        className="flex items-center justify-between w-full px-6 md:px-8 py-5 text-sm font-semibold uppercase tracking-wider text-black group-hover:text-white transition-colors"
                      >
